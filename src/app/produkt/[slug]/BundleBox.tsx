...
function resolveItems(bundle: Product) {
...
function sumSeparately(items: { product: Product; quantity: number }[]) {
  return items.reduce((s, i) => s + (i.product.priceFrom ?? 0) * i.quantity, 0)
}
  ...
  return (
    <section id="w-zestawie">
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-4">
        ...
        <p className="text-sm text-gray-500">Trzy pozycje, jedna cena, jedna przesyłka.</p>
      </div>
        ...
        {items.map(({ product, quantity }) => (
          <article key={product.id} className="rounded-xl border border-slate-200 bg-white p-4 flex flex-col">
            <Link href={`/produkt/${product.slug}`} className="relative block h-36 w-full overflow-hidden rounded-lg bg-white">
              <Image src={product.images[0]} alt={product.name} fill sizes="(min-width: 768px) 30vw, 100vw" className="object-contain p-3" />
            </Link>
            ...
            {product.priceFrom ? (
              <p className="mt-auto pt-3 text-sm text-gray-500">
                <span className="font-medium text-gray-900 tabular-nums">{fmt(product.priceFrom * quantity)} zł</span> netto
              </p>
            ) : null}
          </article>
        ))}
      ...
      {bundle.priceFrom ? (
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-4 sm:px-6 grid gap-4 sm:grid-cols-3 sm:items-center">
          <div>
            <p className="text-xs uppercase tracking-wide text-gray-500">Suma osobno</p>
