...
import { PlusIcon, CheckIcon, ShoppingCartIcon } from '@/components/ui/Icons'
...
export default function SmartPrice({ product, badge }: SmartPriceProps) {
  ...
  const { addItem, updateQuantity, isInCart, openDrawer, getItem } = useCartStore()
  ...
  return (
    <div className="relative z-[1] bg-gray-100 shadow-sm rounded-xl p-4 sm:p-6 mb-6">
      ...
      {/* Rabat ilościowy (sprzedaż na sztuki, np. karty zbliżeniowe): cena za sztukę od progu,
            ...
            {tiers.map((t) => (
              <li key={t.minQty} className="flex items-center justify-between gap-3 py-1.5 flex-wrap">
                ...
                <span className="flex items-center gap-3">
                  ...
                  >
                    {tierQtyInCart === t.minQty ? <CheckIcon size={18} /> : <ShoppingCartIcon size={18} />}
                  </button>
                </span>
              </li>
            ))}
      ...
      {!loading && price && (
        <div className="mt-3 pt-3 border-t border-gray-200">
          ...
          <div className="mt-1.5 h-1.5 w-full rounded-full bg-gray-200 overflow-hidden" aria-hidden="true">
            <div
