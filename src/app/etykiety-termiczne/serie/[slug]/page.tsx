            ...
            <tbody className="divide-y divide-slate-100">
              {series.techSpecs.map((spec, i) => (
                <tr key={i} className="hover:bg-slate-50">
                  ...
                  <td className="p-3 text-gray-900">{spec.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {series.faq.map((q, i) => (
            ...
            >
              <summary className="cursor-pointer list-none flex items-start justify-between gap-4">
                <span className="font-semibold text-gray-900 flex-1">{q.question}</span>
                <ChevronRightIcon
                  size={20}
                  className="text-slate-400 flex-shrink-0 mt-0.5 group-open:rotate-90 transition-transform"
                />
              </summary>
              <p className="mt-3 text-gray-700 leading-relaxed">{q.answer}</p>
            </details>
          ))}
        </div>
      </section>
